// const suggestion = `>His dedication to becoming Hokage inspires everyone around him. His unwavering spirit and the bonds he forms with friends and rivals alike are the heart of the series.
...
export function parseSuggestion(suggestion: Suggestion) {
  const firstContent = suggestion.choices[0].message.content;

  if (!firstContent) {
    throw new Error("No content found in suggestion");
  }

  const splitByNewLine = firstContent.split("\n");
  const stringsExcludingSymbols = splitByNewLine.map((string) =>
    string.replace(">", "").trim()
  );
  return stringsExcludingSymbols;
}
